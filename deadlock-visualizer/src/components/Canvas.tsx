...
function Edge({ edge, nodeMap, isHighlighted, isDeadlockEdge, onRemove }: EdgeProps) {
  ...
  // Determine color priority: cycle highlight > DFS highlight > type color
  let strokeColor: string;
  let glowColor: string;
  if (isDeadlockEdge || isHighlighted) {
    strokeColor = '#ef4444'; // red for deadlock cycle
...
function NodeRenderer({ node, isDeadlocked, isSelected, isHighlighted, onPointerDown, onClick, onContextMenu }: NodeRendererProps) {
  ...
  return (
    ...
    >
      ...
      {isProcess ? (
        <>
          {/* Outer glow ring for deadlocked/selected */}
        ...
        <>
          {(isSelected || isHighlighted) && (
            <rect
              x={-5} y={-5} width={RES_SIZE + 10} height={RES_SIZE + 10}
              rx={20} fill="none"
          ...
          <rect
            x={0} y={0}
            width={RES_SIZE} height={RES_SIZE}
            rx={14}
            fill={`url(#rg-${node.id})`}
            stroke={stroke}
            strokeWidth={strokeWidth}
          ...
          <circle cx={RES_SIZE / 2} cy={RES_SIZE / 2 + 14} r={11} fill="#0c2340" stroke="#1e4a8f" strokeWidth={1.5} pointerEvents="none" />
          <text
            x={RES_SIZE / 2} y={RES_SIZE / 2 + 19}
            textAnchor="middle"
            fontSize="12"
            fontWeight="700"
            fill="#38bdf8"
...
function Canvas() {
  ...
  // Which edges are in a deadlock cycle? (from/to deadlocked nodes and each other)
  ...
  const handlePointerMove = useCallback(
    (event: React.PointerEvent<SVGSVGElement>) => {
      ...
      const x = event.clientX - rect.left - dragState.offsetX + nodes.find((n) => n.id === dragState.id)!.x;
      const y = event.clientY - rect.top - dragState.offsetY + nodes.find((n) => n.id === dragState.id)!.y;
      // Correct offset calculation using SVG-relative coords
      const svgX = event.clientX - rect.left;
      const svgY = event.clientY - rect.top;
      updateNodePosition(
        dragState.id,
        Math.max(40, Math.min(svgX - dragState.offsetX, rect.width - 80)),
        Math.max(40, Math.min(svgY - dragState.offsetY, rect.height - 80))
      );
    },
    [dragState, updateNodePosition, nodes]
  );

  const handlePointerMoveFixed = useCallback(
    (event: React.PointerEvent<SVGSVGElement>) => {
      if (!dragState) return;
      const svg = event.currentTarget;
      const rect = svg.getBoundingClientRect();
  ...
  const handleNodeContextMenu = useCallback(
    (event: ReactMouseEvent, node: GraphNode) => {
      ...
      if (node.type === 'resource') {
        updateResourceCount(node.id, event.shiftKey ? -1 : 1);
      } else {
        removeNode(node.id);
      }
    },
    ...
    <div className="relative overflow-hidden rounded-3xl border border-white/5 bg-slate-950 shadow-2xl" style={{ height: 560 }}>
      ...
      {walkthroughSteps.length > 0 && currentStep && (
        ...
        >
          ...
          `} />
          <span className="max-w-[500px] truncate">{currentStep.message}</span>
          <span className="ml-2 text-slate-500">
            {activeWalkthroughStepIndex + 1}/{walkthroughSteps.length}
          </span>
        </div>
      )}
      ...
      <svg
        ...
        onPointerMove={handlePointerMoveFixed}
      ...
      >
        ...
        {/* Selection indicator line while connecting */}
        {selectedSourceId && (
          <g>
            {nodeMap[selectedSourceId] && (
              <circle
                ...
                r={nodeMap[selectedSourceId].type === 'process' ? NODE_R + 10 : RES_SIZE / 2 + 10}
                fill="none"
                stroke="#38bdf8"
                strokeWidth={2}
                strokeDasharray="5 3"
